...
class CooksProblemForwardSoft(Problem):
    ...
    def loss_fn(all_params, constraints):
        ...
        dbc_loss_left = jnp.mean(ux**2) + jnp.mean(uy**2) 
        ...
        return phy_loss + nbc_loss_right + nbc_loss_top + dbc_loss_left + nbc_loss_bottom
...
#################################################################################
######################### Soft Boundary Condition V2############################
#################################################################################
    
class CooksProblemForwardSoftVersion2(Problem):
    """ Linear Elasticity, plain strain, DBC at left, NBC at right, 0 traction at the bottom and top"""
    @staticmethod
    def init_params(lambda_true= 4, mu_true = 5, 
                    nbc_points_right = 10, nbc_points_top=10, 
                    nbc_points_bottom=10, dbc_points_left=10, sd=0.1,
                    weight_mm_1 = 1e0, weight_mm_2=1e0, weight_mm_3=1e0,
                    weight_bl_1=1e0, weight_bl_2=1e0, 
                    weight_nbc_top=2e6, weight_nbc_bottom=1e6, weight_nbc_right=1e6,
                    weight_dbc_left=1e6):
        ...
        static_params = {
            ...
            "dbc_points_left":dbc_points_left,
            "sd":sd,
            "weight_mm_1":weight_mm_1,
            "weight_mm_2":weight_mm_2,
            "weight_mm_3":weight_mm_3,
            "weight_bl_1":weight_bl_1,
            "weight_bl_2":weight_bl_2,
            "weight_nbc_top":weight_nbc_top,
            "weight_nbc_bottom":weight_nbc_bottom,
            "weight_nbc_right":weight_nbc_right,
            "weight_dbc_left":weight_dbc_left,
        }
    ...
    def loss_fn(all_params, constraints):
        ...
        weight_mm_1 = all_params["static"]["problem"]["weight_mm_1"]
        weight_mm_2 = all_params["static"]["problem"]["weight_mm_2"]
        weight_mm_3 = all_params["static"]["problem"]["weight_mm_3"]
        weight_bl_1 = all_params["static"]["problem"]["weight_bl_1"]
        weight_bl_2 = all_params["static"]["problem"]["weight_bl_2"]
        weight_nbc_top = all_params["static"]["problem"]["weight_nbc_top"]
        weight_nbc_bottom = all_params["static"]["problem"]["weight_nbc_bottom"]
        weight_nbc_right = all_params["static"]["problem"]["weight_nbc_right"]
        weight_dbc_left = all_params["static"]["problem"]["weight_dbc_left"]
        ...
        MMxy = (E / (1+nu) ) *  epsilon_xy 
        MM_loss = (weight_mm_1 * jnp.mean((sigmaxx - MMx)**2) + 
                    weight_mm_2 * jnp.mean((sigmayy - MMy)**2) + 
                    weight_mm_3 * jnp.mean((sigmaxy - MMxy)**2))
        # Balance
        """
        Balance Loss
        (1) σ₁₁,₁ + σ₁₂,₂ = 0
        (2) σ₂₁,₁ + σ₂₂,₂ = 0           # Note: σ₁₂ == σ₂₁ (symmetric)
        """
        Balance_loss = ( weight_bl_1 * jnp.mean((sigmaxx_x + sigmaxy_y)**2) + 
                        weight_bl_2 * jnp.mean((sigmayy_y + sigmaxy_x)**2) )
        phy_loss = MM_loss + Balance_loss
        ...
        unit_normal_vec_right = jnp.array([1.,0.])                      # unit normal (1,0)
        applied_traction_right = jnp.array([0.,1.])
        nbc_loss_right = (weight_nbc_right * (jnp.mean((sigmaxx_n*unit_normal_vec_right[0] + sigmaxy_n*unit_normal_vec_right[1] - applied_traction_right[0])**2) +
                    jnp.mean((sigmaxy_n*unit_normal_vec_right[0] + sigmayy_n*unit_normal_vec_right[1] - applied_traction_right[1])**2)) )
        ...
        unit_normal_vec_top = jnp.array([-0.3162279,  0.9486832])        # unit normal (-0.3162279,  0.9486832) 
        applied_traction_top = jnp.array([0.,0.])
        nbc_loss_top = ( weight_nbc_top * (jnp.mean((sigmaxx_n*unit_normal_vec_top[0] + sigmaxy_n*unit_normal_vec_top[1] - applied_traction_top[0])**2) +
                    jnp.mean((sigmaxy_n*unit_normal_vec_top[0] + sigmayy_n*unit_normal_vec_top[1] - applied_traction_top[1])**2)) )
        
        # dbc left
        _, ux, uy = constraints[3]
        dbc_loss_left = weight_dbc_left * ( jnp.mean(ux**2) + jnp.mean(uy**2) )
        ...
        applied_traction_bottom = jnp.array([0.,0.])
        nbc_loss_bottom = (weight_nbc_bottom * (jnp.mean((sigmaxx_n*unit_normal_vec_bottom[0] + sigmaxy_n*unit_normal_vec_bottom[1] - applied_traction_bottom[0])**2) +
                    jnp.mean((sigmaxy_n*unit_normal_vec_bottom[0] + sigmayy_n*unit_normal_vec_bottom[1] - applied_traction_bottom[1])**2)) )
...
class CooksProblemForwardHard(Problem):
    ...
    def constraining_fn(all_params, x_batch, solution):
        ...
        # u = solution[:, 0:1] * tanh(x/sd) * tanh((0.044 - y)/sd) # Hard constraining (DBC, ux=0, at x=0) 
        # v = solution[:, 1:2] * tanh(x/sd) * exp(x/sd) # DBC, uy=0, at x=0
        # Hard BC on stress
        tolerence = 1e-8
        G_sigmaxx = 0.0         # Boundary Extension
        D_sigmaxx = 0.048 - x   # Distance Function
        # D_sigmaxx = tanh((0.048 - x )) * tanh(0.625*x + 0.022 - y) 
        sigmaxx = G_sigmaxx +  D_sigmaxx * sigmaxx
        G_sigmaxy = 1   # [traction at the right boundary = [0,1]]
        D_sigmaxy = 0.048 - x 
        # D_sigmaxy = tanh((0.048 - x )) * tanh(0.625*x + 0.022 - y)
    ...
    def loss_fn(all_params, constraints):
        ...
        """
        Balance_loss =  jnp.mean((sigmaxx_x + sigmaxy_y)**2) + jnp.mean((sigmayy_y + sigmaxy_x)**2)
        phy_loss = 1e2*MM_loss + 1e3*Balance_loss
